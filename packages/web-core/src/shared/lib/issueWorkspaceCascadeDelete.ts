 ...
 * their corresponding remote workspace records, on a best-effort basis.
 * Failures (e.g. a still-running workspace) are surfaced to the user without
 * blocking the caller's own deletion flow.
 */
export async function deleteWorkspacesBestEffort(
  ...
  for (const workspace of workspaces) {
    try {
      // Local delete clears the remote workspace record by default.
