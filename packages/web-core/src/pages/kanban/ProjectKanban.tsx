...
function useFindProjectById(projectId: string | undefined) {
  ...
  const organizationId =
    project?.organization_id ?? selectedOrgId ?? organizations[0]?.id ?? null;

  // Sidebar navigation already has org + project context cached; only block
  // when we still cannot resolve which org owns this project.
  const isLoading =
    !authLoaded ||
    (!organizationId && orgsLoading) ||
    (!project && !selectedOrgId && projectsLoading);
